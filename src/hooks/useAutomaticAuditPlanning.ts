...
import { addMonths, differenceInMonths, isBefore, isAfter, addDays } from 'date-fns';
  ...
  const suggestions = useMemo(() => {
    ...
    for (const cert of certifications) {
      ...
      if (completedAudits.length > 0) {
        ...
        if (monthsSinceLastAudit >= 10 && monthsUntilExpiry > 4) {
          const hasSurveillanceScheduled = scheduledAudits.some(a => a.type === 'surveillance');
          
          if (!hasSurveillanceScheduled) {
